// 原 WTangent.Client/Store/ServerRegistry.cs，跨仓去重并入 Core（逻辑未改）
using System.Text.Json;

namespace WTangent.Core;

// RemoteEntry 不重复造：与既有 WTangent.Core.RemoteEntry（App/IRemoteClient.cs）并型，EtCode/Kind/Url 已并入该处。
...
/// 读写优先走宿主注入的 IAppStore（构造注入；统一原子写 + 变更事件），未注入时回退直接文件访问。</summary>
public sealed class ServerRegistry(string? path = null, IAppStore? store = null)
{
    ...
    private IAppStore? AppStore => store;
